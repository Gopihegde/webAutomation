...
public class CreateNewPageComponent {
  ...
  // Create New page from here.
    ...
    public void dragMailToConsole(){
        ...
        sendEmailfromMenu.click();
        wait.until(ExpectedConditions.visibilityOf(sendEmailfromMenu));

        }

    public void connectStartToSendMessage() {
        wait.until(ExpectedConditions.visibilityOf(dragSMS));
        if(dragStartPointfromStart.isEnabled())
            System.out.println("hjajaj");
        wait.until(ExpectedConditions.visibilityOf(dragEndToSMS));
        if (dragEndToSMS.isEnabled())
